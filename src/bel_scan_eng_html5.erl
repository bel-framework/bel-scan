...
-record(state, { attr_engines :: [module()] }).
...
init(Opts) ->
        ...
        state = #state{
            attr_engines = bel_scan:init_engines(
                maps:get(attr_engines, Opts, ?ATTRS_ENGINES)
            )
        }
    }.
...
handle_match({?MODULE, special_tag, Captured, Anno}, Scan) ->
    ...
    Token = tag_token(Tag, Anno, Metadata),
    {reply, [Token], Scan};
...
handle_match({?MODULE, elem_tag, Captured, Anno}, Scan) ->
    ...
    Token = tag_token(Tag, Anno, Metadata),
    {reply, [Token], Scan};
handle_match({?MODULE, comment, _Captured, _Anno}, Scan) ->
...
attributes(Bin, PrevParts, Anno, Scan) ->
    ...
    bel_scan:get_tokens(bel_scan:bin(Bin, #{
        engines => State#state.attr_engines,
      ...
      , ?assertEqual([
            ...
            {token,html_tag,
                ...
                {[{token,html5_attribute,
                    ...
                    {loc,42,1,43,1,1},
                    <<"hidden">>},
                    <<"hidden">>,bel_scan_eng_html5_attr}],
                []}},
                bel_scan_eng_html5},
            {token,text,
            ...
            {token,html_tag,
                ...
                {token,html5_attribute,
                    ...
                    {loc,175,1,176,1,1},
                    <<"required">>},
                    <<"required">>,bel_scan_eng_html5_attr}],
                <<"<b>AAA</b>">>}},
                bel_scan_eng_html5},
            {token,text,
                {anno,string,
    ...
    , { "Should scan multiple lines"
      , ?assertEqual([
            ...
            {token,html_tag,
                ...
                bel_scan_eng_html5},
            {token,text,
                {anno,string,{loc,40,6,10,1,1},{loc,41,7,1,1,1},<<"\n">>},
                undefined,bel_scan},
            {token,html_tag,
                {anno,string,
                {loc,41,7,1,1,1},
